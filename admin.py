...
def adminviewfarmers():
    ...
    if action == "accept":
        ...
        html_content = render_template('confirmation_email.html', username="miniprojects4bca@gmail.com")
        if send_email('Registration Response From Admin', email, pwd, html_content):
    ...
    if action == "reject":
        ...
        q="select * from farmer where login_id='%s'"%(login_id)
        email=select(q)[0]['email']
        pwd = "Your Registration request has been Rejected, Try again later!"
        html_content = render_template('rejection_mail.html', username="miniprojects4bca@gmail.com")
        if send_email('Registration Response From Admin', email, pwd, html_content):
            flash("Email sent successfully!")
        else:
            flash("Error sending email.")
...
def admin_manage_pest():
    ...
    if 'btn' in request.form:
        ...
        q = "INSERT INTO pest VALUES (NULL, '%s', '%s', '%s')" % (
            escape_quotes(pest), escape_quotes(details), escape_quotes(path)
        )
